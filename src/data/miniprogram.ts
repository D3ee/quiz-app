...
export const miniprogramQuestions: Question[] = [
  ...
  // ==================== 补充题目 (461-472) ====================
  {
    ...
    options: ['频繁调用 setData 更新单个字段', '一次 setData 更新多个字段', '使用数据路径更新数组某一项', '延迟批量更新'],
    answer: 0,
  ...
  },
  {
    ...
    options: ['使用虚拟列表（recycle-view）', '分页加载数据', '图片懒加载', '避免在列表项中使用复杂计算'],
    answer: [0, 1, 2, 3],
  ...
  },
  {
    ...
    options: ['没有区别', 'catchtap 会阻止事件冒泡', 'bindtap 会阻止事件冒泡', 'catchtap 只能用于自定义组件'],
    answer: 1,
  ...
  },
  {
    ...
    options: ['没有区别', 'target 是触发事件的源组件，currentTarget 是绑定事件的组件', 'currentTarget 是触发事件的源组件，target 是绑定事件的组件', 'target 只在冒泡阶段存在'],
    answer: 1,
  ...
  },
  {
    ...
    options: ['组件样式不受外部影响，外部样式也不影响组件内', '组件样式可以影响外部', '外部样式可以影响组件内', '完全禁用样式'],
    answer: 0,
  ...
  },
  {
    ...
    options: ['multipleSlots: true', 'slots: true', 'namedSlots: true', '不需要配置'],
    answer: 0,
  ...
  },
  {
    ...
    options: ['服务端渲染', '在视图层操作 DOM 和运行第三方库', '编译时优化', '云函数调用'],
    answer: 1,
  ...
  },
  {
    ...
    options: ['手动删除重新安装', 'HBuilderX 的插件更新功能', 'npm update', 'git pull'],
    answer: 1,
  ...
  },
  {
    ...
    options: ['app.json', 'project.config.json', '小程序管理后台', 'manifest.json'],
    answer: 2,
  ...
  },
  {
    ...
    options: ['使用分包加载', '压缩图片资源', '清理无用代码和文件', '使用 CDN 加载大文件'],
    answer: [0, 1, 2, 3],
  ...
  },
  {
    ...
    options: [
      '使用 wx.chooseImage 选择图片，直接上传原图',
      '使用 wx.chooseImage 并设置 sizeType: ["compressed"]，再使用 wx.uploadFile 上传',
      '使用 canvas 手动压缩图片',
      '使用第三方压缩库',
    ],
    answer: 1,
  ...
  },
  {
    ...
    options: [
      '在页面中定义 onShareAppMessage 函数',
      '设置 button 的 open-type="share" 触发分享',
      '可以自定义分享标题、图片和路径',
      '可以直接分享到朋友圈',
    ],
    answer: [0, 1, 2],
    explanation: '小程序分享：页面定义 onShareAppMessage 返回分享配置（title、path、imageUrl），或使用 button 的 open-type="share" 触发。分享到朋友圈需要满足特定条件且使用 onShareTimeline。',
  },
  {
    id: '473-miniprogram',
    type: 'single',
    category: 'miniprogram',
    question: '微信小程序中，以下哪个 API 用于获取用户的地理位置？',
    options: ['wx.getLocation', 'wx.chooseLocation', 'wx.openLocation', 'wx.getPosition'],
    answer: 0,
    explanation: 'wx.getLocation 用于获取当前地理位置（经纬度）。wx.chooseLocation 打开地图选择位置；wx.openLocation 使用微信内置地图查看位置；没有 wx.getPosition 这个 API。',
  },
  {
    id: '474-miniprogram',
    type: 'single',
    category: 'miniprogram',
    question: 'UniApp 中，如何在不同平台使用不同的样式？',
    options: [
      '写多个 CSS 文件',
      '使用条件编译 /* #ifdef H5 */ 样式 /* #endif */',
      '使用 JavaScript 动态修改样式',
      '不支持平台差异化样式',
    ],
    answer: 1,
    explanation: 'UniApp 的条件编译在 CSS 中也适用。使用 /* #ifdef 平台标识 */ 和 /* #endif */ 包裹平台特定样式。也可以创建平台特定的样式文件如 index.h5.css。',
  },
  {
    id: '475-miniprogram',
    type: 'single',
    category: 'miniprogram',
    question: '微信小程序中，自定义组件的生命周期 attached 对应页面的哪个生命周期？',
    options: ['onLoad', 'onShow', 'onReady', '没有对应关系'],
    answer: 3,
    explanation: '组件生命周期和页面生命周期是独立的。attached 在组件实例进入页面节点树时触发，可能在页面 onLoad 之后、onReady 之前的任意时刻，取决于组件何时被渲染。',
  },
  {
    id: '476-miniprogram',
    type: 'single',
    category: 'miniprogram',
    question: '微信小程序中，以下哪个方法可以停止下拉刷新的动画？',
    options: ['wx.stopPullDownRefresh', 'wx.hidePullDownRefresh', 'wx.closePullDownRefresh', 'wx.endPullDownRefresh'],
    answer: 0,
    explanation: 'wx.stopPullDownRefresh() 用于停止当前页面的下拉刷新。需要在页面配置中设置 enablePullDownRefresh: true，并在 onPullDownRefresh 中处理刷新逻辑。',
  },
  {
    id: '477-miniprogram',
    type: 'multiple',
    category: 'miniprogram',
    question: 'UniApp 中，以下哪些是有效的跨端兼容策略？（多选）',
    options: [
      '使用条件编译区分平台代码',
    ...
    answer: [0, 1, 2, 3],
    explanation: '这些都是 UniApp 跨端开发的有效策略。条件编译在编译期区分平台；运行时判断平台；flex 布局各平台支持良好；避免平台特有 API 或用条件编译包裹。',
  },
  {
    id: '478-miniprogram',
    type: 'single',
    category: 'miniprogram',
    question: '微信小程序中，Component 构造器的 options 字段中，multipleSlots 的作用是？',
    options: [
      '允许组件有多个根节点',
      '启用多个插槽支持',
      '允许多个实例',
    ...
    answer: 1,
    explanation: '设置 options: { multipleSlots: true } 后，组件可以使用多个具名插槽。在 WXML 中使用 <slot name="header"> 定义，使用时通过 slot="header" 指定内容插入位置。',
  },
  {
    id: '479-miniprogram',
    type: 'single',
    category: 'miniprogram',
    question: '【场景题】UniApp 项目需要调用原生插件（如蓝牙、NFC），应该如何处理？',
    options: [
      'UniApp 不支持原生插件',
      '使用 uni 原生插件（uni_modules）或条件编译调用平台特定 API',
      '只能用原生开发',
      '使用 WebView 嵌入',
    ],
    answer: 1,
    explanation: 'UniApp 支持原生插件开发（uni_modules 规范）。也可以使用条件编译，在不同平台调用对应的原生 API（如 App 端调用 plus API，小程序端调用 wx API）。',
  },
  {
    id: '480-miniprogram',
    type: 'single',
    category: 'miniprogram',
    question: '【场景题】小程序需要实现"长按保存图片"功能，最佳实现方式是？',
    options: [
      '使用 image 标签的 show-menu-by-longpress 属性',
      '监听 longpress 事件手动实现',
      '使用 canvas 绘制图片',
      '小程序不支持保存图片',
    ],
    answer: 0,
    explanation: 'image 组件的 show-menu-by-longpress="true" 属性可以开启长按图片显示识别小程序码菜单。也可以监听 longpress 事件，调用 wx.previewImage 预览后用户可以保存，或使用 wx.saveImageToPhotosAlbum 直接保存。',
  },
]
